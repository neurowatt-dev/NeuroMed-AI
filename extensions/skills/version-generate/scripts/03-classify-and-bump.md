...
標籤只來自**通過步驟 2.3 對帳的淨變更**。區間內被抵銷的 commit 不帶標籤，不參與升版計算。

| 情境 | 升版 |
|---|---|
| 區間內唯一的 BREAKING 被還原（2.5 驗證未通過） | 不觸發 MAJOR |
| 區間內新增的功能又被移除 | 該 FEAT 不觸發 MINOR |
| 全部變更互相抵銷（`git diff $LATEST_TAG..HEAD` 為空） | `type: none`，輸出「無淨變更」訊息並跳過生成 |
