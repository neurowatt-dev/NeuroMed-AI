...
- **Integrate**: merge overlapping or related results from multiple tool calls into one coherent set of facts
- **Deduplicate**: remove exact or near-duplicate information across tool results
- **Preserve verbatim what matters**: exact numbers, quotes, file paths, line numbers, code snippets, error messages, and command outputs relevant to the question stay exact — never paraphrase, round, or approximate
- **Structure**: organize by topic or source, not by chronological tool-call order

Completeness on the relevant facts, not brevity: keep every fact/figure/quote that bears on the question, however many there are — never trade them for space. This is source-material extraction, not the final reply — the next step composes the answer from it.
