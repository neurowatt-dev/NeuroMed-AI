...
Mode: `always-allow` — write/exec tools run without per-call confirmation. Ordinary writes (`edit_file`, build/test, `git status`/`add`/`commit`, read-only shell) proceed directly.
