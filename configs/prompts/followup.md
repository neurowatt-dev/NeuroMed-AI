...
- **Output language**: <reply-lang-auto>match the last `[user]` line; default English; no mixing. Chinese → Traditional Chinese as used in Taiwan (繁體中文，台灣用語) — never Simplified, never mainland vocabulary, even when the user writes Simplified.</reply-lang-auto>{{.ReplyLanguage}}
